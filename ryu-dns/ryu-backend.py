...
HEARTBEAT_TIMEOUT = 10 #5 seconds
...
class BackendDatapath(controller.Datapath):
    ...
    def handle_event(self, header, msg):
        ...
        if msg:
            # print "it's going to msg_to_ev!!!"
            ev = ofp_event.ofp_msg_to_ev(msg)
            print "event is :",ev
            event_observers = self.ofp_brick.get_observers(ev,self.state)
            print "event observer is :",event_observers
            print "running_modules:",self.channel.running_modules
            ...
            for key, value in self.channel.running_modules.iteritems():
                if value == module_id and key in event_observers:
                    ...
                    print "module_brick",module_brick
                    print "handlers are :",module_brick_handlers
                    for handler in module_brick_handlers:
                        print "whuwzp:it's going to handle event"
                        handler(ev)
                    break
...
class CoreConnection(threading.Thread):
    ...
    def backend_announcement(self, backend):
        ...
        # print "whuwzp : ann_message is ",ann_message
    ...
    def module_announcement(self, bricks, backend):
        ...
        for key,value in bricks.iteritems():
            ...
            if key is not "ofp_event" and key is not self.backend.name: #we take only the control applications
                ...
                # print "ann_message is ",ann_message
                self.socket.send(ann_message)
                ack = False
                while ack is False:
                    ack_message = self.socket.recv_multipart()
                    ...
                    if message_type is NetIDEOps.NetIDE_type['NETIDE_MODULE_ACK'] and message_data == module_name:
                        logger.debug( "Received ack from Core header: %s" , decoded_header)
...
class RyuBackend(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.backend_name = b"NOS-" + b"LOCATION-" + b"Ryu-"+ str(os.getpid())
        self.backend_id = 0
